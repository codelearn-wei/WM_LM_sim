...
class XYTrafficDataset(Dataset):
    """
    ...
    def _create_vehicle_sample(self, ego_id, data_by_timestamp, vehicle_data_by_ts_id, 
        ...
        # 注意：修改特征维度为2 (rel_x, rel_y, is_lane_changing, is_main_lane)
        total_steps = self.history_steps + self.future_steps
        trajectory = np.zeros((total_steps, self.max_nearby_vehicles, 2), dtype=np.float32)
        ...
        for t_idx, timestamp in enumerate(all_timestamps):
            ...
            for v_idx, rel_vehicle in enumerate(nearest_vehicles):
                ...
                # 批量填充特征 - 减少循环内部的索引操作
                # 只使用x, y, is_lane_changing, is_main_lane特征
                features = np.array([
                    rel_vehicle['rel_x'],
                    rel_vehicle['rel_y']
                ], dtype=np.float32)
        ...
        return {
            'ego_id': ego_id,
            'history_trajectory': history_trajectory,  # 形状: (history_steps, max_nearby_vehicles, 2)
            'future_trajectory': future_trajectory,    # 形状: (future_steps, max_nearby_vehicles, 2)
