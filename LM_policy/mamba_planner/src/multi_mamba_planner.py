...
class HistoryFutureAttention(nn.Module):
    """历史轨迹和未来轨迹之间的注意力模块"""
    def __init__(self, d_model, num_heads=4, dropout=0.1):
        super().__init__()
    ...
    def forward(self, history, future=None):
        """
        历史和未来轨迹间的时序注意力
        Args:
            history: (B, T_hist, N, D) - 历史轨迹特征
            future: (B, T_fut, N, D) - 未来轨迹特征，如果为None则为自注意力
        Returns:
            out: 与输入形状相同
        """
        B, T_hist, N, D = history.shape
        
        # 将历史轨迹重塑为序列
        hist_flat = history.permute(0, 2, 1, 3).reshape(B*N, T_hist, D)  # (B*N, T_hist, D)
        
        if future is None:
            # 自注意力模式
            attn_out, _ = self.self_attn(hist_flat, hist_flat, hist_flat)
            attn_out = self.dropout1(attn_out)
            attn_out = self.norm1(attn_out + hist_flat)  # 残差连接
            
            # 前馈网络
            ffn_out = self.ffn(attn_out)
            out = self.norm2(ffn_out + attn_out)  # 残差连接
            
            # 恢复原始形状
            out = out.view(B, N, T_hist, D).permute(0, 2, 1, 3)  # (B, T_hist, N, D)
            return out
        else:
            # 交叉注意力模式
            T_fut = future.size(1)
            fut_flat = future.permute(0, 2, 1, 3).reshape(B*N, T_fut, D)  # (B*N, T_fut, D)
            
            # 历史作为查询，未来作为键和值
            attn_out, _ = self.self_attn(hist_flat, fut_flat, fut_flat)  # (B*N, T_hist, D)
            attn_out = self.dropout1(attn_out)
            attn_out = self.norm1(attn_out + hist_flat)  # 残差连接
...
class TrajectoryEncoder(nn.Module):
    def __init__(self, in_features, d_model, agent_type_num=None, use_agent_type=True, num_attn_layers=2):
        ...
        # 新增：历史轨迹自注意力
        self.history_self_attn = HistoryFutureAttention(d_model)
    ...
    def forward(self, x):
        ...
        # 应用历史自注意力
        x = self.history_self_attn(x)
            
        return x

class MultiModalTrajectoryDecoder(nn.Module):
    """增强版多模态轨迹解码器，带历史-未来注意力"""
    def __init__(self, d_model, hist_len, pred_len, output_dim=2, num_modes=3, num_layers=2):
        """
        Args:
            d_model: 模型维度
            hist_len: 历史轨迹长度
        ...
        self.mode_decoders = nn.ModuleList([
            ModalDecoder(d_model, hist_len, pred_len, output_dim, num_layers) 
    ...
    def forward(self, encoded_hist):
        """
        Args:
            encoded_hist: (B, T_hist, N, D) - 编码后的历史轨迹
        Returns:
            trajectories: (B, N, M, T_pred, output_dim) - 多模态轨迹预测
            confidences: (B, N, M) - 各模态置信度
        """
        B, T_hist, N, D = encoded_hist.shape
        
        # 使用最后一个时间步为模态选择
        last_step = encoded_hist[:, -1]  # (B, N, D)
        
        # 计算每个模态的置信度
        confidences = self.mode_selection_head(last_step)  # (B, N, M)
        confidences = functional.softmax(confidences, dim=-1)
        
        # 为每个模态生成轨迹
        modal_trajectories = []
        for i in range(self.num_modes):
            traj = self.mode_decoders[i](encoded_hist)  # (B, N, T_pred, output_dim)
            modal_trajectories.append(traj)
        
        # 堆叠所有模态的轨迹
        trajectories = torch.stack(modal_trajectories, dim=2)  # (B, N, M, T_pred, output_dim)
        
        return trajectories, confidences

class ModalDecoder(nn.Module):
    """单个模态的轨迹解码器"""
    def __init__(self, d_model, hist_len, pred_len, output_dim=2, num_layers=2):
        super().__init__()
        self.hist_len = hist_len
        self.pred_len = pred_len
        self.output_dim = output_dim
        
        # 初始化预测轨迹的起始embeddings
        self.future_embeddings = nn.Parameter(torch.randn(1, pred_len, 1, d_model))
        self.pos_enc = PositionalEncoding(d_model)
        
        # 历史-未来注意力层
        self.attn_layers = nn.ModuleList([
            HistoryFutureAttention(d_model) for _ in range(num_layers)
        ])
    ...
    def forward(self, encoded_hist):
        ...
        Args:
            encoded_hist: (B, T_hist, N, D) - 编码后的历史轨迹
        Returns:
            pred_traj: (B, N, T_pred, output_dim) - 预测轨迹
        """
        B, T_hist, N, D = encoded_hist.shape
        
        # 初始化未来轨迹embeddings
        future_emb = self.future_embeddings.expand(B, self.pred_len, N, D)  # (B, T_pred, N, D)
        future_emb = self.pos_enc(future_emb)
        
        # 多层历史-未来注意力
        hist_context = encoded_hist
        fut_context = future_emb
        
        for i, attn_layer in enumerate(self.attn_layers):
            if i % 2 == 0:
                # 奇数层：历史作为查询，未来作为键/值
                hist_context = attn_layer(hist_context, fut_context)
            else:
                # 偶数层：未来作为查询，历史作为键/值
                fut_context = attn_layer(fut_context, hist_context)
        
        # 投影到输出空间
        pred_traj = self.output_proj(fut_context)  # (B, T_pred, N, output_dim)
        
        # 调整维度顺序为 (B, N, T_pred, output_dim)
        pred_traj = pred_traj.permute(0, 2, 1, 3)
        
        return pred_traj

class EnhancedTrajPlanningModel(nn.Module):
    """增强版轨迹规划模型，包含历史-未来注意力"""
    def __init__(self, 
                 in_features, 
                 hist_len,
                 pred_len,
        ...
        # 多模态轨迹解码器
        self.decoder = MultiModalTrajectoryDecoder(
            d_model=d_model,
            hist_len=hist_len,
            pred_len=pred_len,
    ...
    def forward(self, x):
        """
        Args:
            x: (B, T_hist, N, F) - 历史轨迹输入
        Returns:
            trajectories: (B, N, M, T_pred, output_dim) - 多模态轨迹预测
            confidences: (B, N, M) - 各模态置信度
        """
        # 编码历史轨迹
        encoded_hist = self.encoder(x)
        
        # 解码生成多模态未来轨迹
        trajectories, confidences = self.decoder(encoded_hist)
...
if __name__ == "__main__":
    # 示例参数
    B, hist_len, N, F = 8, 20, 10, 6  # 8批次，20帧历史，10个车，每车6维特征
    pred_len = 30  # 预测30帧
    d_model = 128
    agent_type_num = 4
    num_modes = 3  # 3个模态
    ...
    # 创建增强版模型
    model = EnhancedTrajPlanningModel(
        in_features=F,
        hist_len=hist_len,
        pred_len=pred_len,
        d_model=d_model,
    ...
    # 创建输入 - 历史轨迹
    x = torch.randn(B, hist_len, N, F-1)  # 先创建F-1维特征
    # 为agent_type创建随机整数 (范围是0到agent_type_num-1)
    agent_types = torch.randint(0, agent_type_num, (B, hist_len, N, 1))
    # 拼接特征和类型
